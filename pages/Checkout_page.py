...
class CheckoutPage(BasePage):
    ...
    def verify_total_amount(self, total_item_amt: str):
        ...
        ), f"Expected total '{expected_total}' not found in '{total_price}'"
    ...
    def assert_checkout_details(self):
        expect(self.last_name_input, "Should have last name.").to_have_text(
            self.lastName
        )
        expect(self.first_name_input, "Should have first name.").to_have_text(
            self.firstName
        )
        expect(self.postal_code_input, "Should have postal code.").to_have_text(
            str(self.postalCode)
        )

    def assert_finish_button(self):
        expect(
            self.page, "Navigation to order completion stopped for error user."
        ).to_have_url(TestData.CHECKOUT_URL)
